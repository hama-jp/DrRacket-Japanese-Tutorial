...
5. 解答は [solutions/](solutions/README.md) に章ごとにまとめてある。**ヒント → 部分解答 → 完全解答** の順なので、詰まった段階だけ覗くのがおすすめ
...
- [solutions/](solutions/README.md) — 各章「手を動かしてみよう」の **ヒント → 部分解答 → 完全解答**

## ディレクトリ構成

```
DrRacket-Japanese-Tutorial/
├── README.md           # この目次
├── book/               # 本文(章ごとに Markdown)
├── examples/           # 各章の実行可能サンプル(*.rkt)
│   └── chNN/           # 章番号ごとに格納
├── solutions/          # 章末「手を動かしてみよう」の解答
│   ├── README.md       # 使い方(ヒント → 部分解答 → 完全解答)
│   └── chNN.md         # 章ごとの解答
└── images/             # 図や PNG など
```
