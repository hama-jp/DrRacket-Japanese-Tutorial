...
本書の第 2 版は、A.5 で挙げた「触れなかったトピック」のうち **読者がすぐ楽しめる / すぐ実用に使える** ものから順に章立てします。具体的には以下の方向です。

- **楽しい章**: `2htdp/image` & `2htdp/universe` でゲーム、`plot` でグラフ、`generator` / `stream` で無限を扱う
- **実用の章**: `db` で SQLite、`net/http-easy` + `json` で外部 API、`racket/cmdline` + `raco exe` で CLI ツール配布

詳細な章立て、執筆順、スタイル方針は [v2-roadmap.md](v2-roadmap.md) を参照してください。
