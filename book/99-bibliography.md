...
本書の第 2 版は、A.5 で挙げた「触れなかったトピック」のうち **読者がすぐ楽しめる / すぐ実用に使える** ものから順に章立てします。

- **最初に書く 2 章 (確定)**: `db` で SQLite、`net/http-easy` + `json` で外部 API
- **続く候補**: `plot` でグラフ、`generator` / `stream` で無限、`racket/cmdline` + `raco exe` で CLI ツール配布、ファイル・設定の実務技
- **当面パス**: `2htdp/image` & `2htdp/universe` のゲーム章は実用章を出してから優先度を再判断

詳細な章立て、執筆順、スタイル方針(コードを 5–15 行刻み + 行注釈表で読み解く第 2 版の中核スタイルなど)は [v2-roadmap.md](v2-roadmap.md) を参照してください。

なお当初構想していた「マクロ理論・自作 `#lang`・リーダ拡張」などの **言語拡張の深掘り** は、独立した一冊分のボリュームになるため第 3 版以降で扱う計画に変更しました。
