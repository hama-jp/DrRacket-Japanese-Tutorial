...
3. 意図的にエラーを出してみましょう。
   ...
   (+ 1 "two")
   ```
   出力の `expected: number?` と `given: "two"` を確認してください。Racket のエラーは 3〜5 行あり、英語が混じってパッと見は威圧感があります。けれど 1 語ずつ目で追うと、必ず `expected:`(期待した型)・`given:`(実際に来た値)・`context:`(発生箇所)の **構造に分かれている** ことに気づくはずです。読み慣れてくると、長さではなく「どの情報を見るか」で判断できるようになります。
