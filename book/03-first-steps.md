...
3. 意図的にエラーを出してみましょう。
   ...
   (+ 1 "two")
   ```
   出力の `expected: number?` と `given: "two"` を確認してください。Racket のエラーは複数行あって一見威圧感がありますが、必ず「**何を期待し(expected)・実際に何が来て(given)・どこで起きたか(context)**」の形に分かれています。最初は 1 行ずつ指でなぞるくらいの速さで読むと、怖さが消えて「どの情報を拾えばいいか」が見えてきます。
